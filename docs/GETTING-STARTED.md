...
각자 자기 역할 지시서(`org/roles/*.md`)와 공통 프로토콜을 읽는다. 40초쯤 걸린다.

**이걸 건너뛰면 조직이 아예 생기지 않는다.** 각 자리의 claude 는 자기가 조직의
일원이라는 것을 모른다 — 그냥 그 디렉터리에서 뜬 평범한 Claude Code 세션이다.
대표에게 목표를 말하면 대표는 자기가 대표인 줄 모르니 **혼자 다 만들려 든다.**

부하는 반쯤 움직인다. 배정이 오면 벨 한 줄에 `aiorg inbox` 가 들어 있으니
그것을 따라 일을 시작할 수는 있다. 그런데 프로토콜을 안 읽었으므로
산출물을 엉뚱한 데 만들고, 태스크 상태를 갱신하지 않고, 보고도 올리지 않는다.

빼먹었는지는 `status` 가 알려준다.
...
기동한 뒤 안내를 안 했거나, **같은 자리에서 claude 를 다시 띄운 경우**를 잡는다.
재기동하면 대화 맥락이 사라지므로 그 전의 안내는 무효다.
...
tmux 쪽이 궁금하면 — 누가 어느 세션·창에 앉아 있는지:
...
```
  ...
  aiorg-dev      0   %1    dev-lead  개발팀장           대기
  aiorg-dev      1   %2    dev-1     백엔드 개발자      작업중
  ...
...
한 명이 무엇을 하고 있는지 **붙지 않고** 보려면:

```bash
./aiorg peek dev-1 -n 20
```

`attach` 와 달리 아무것도 입력되지 않으므로 작업 중인 멤버를 방해하지 않는다.
실수로 pane 을 닫을 위험도 없다. 지켜보기만 할 때는 이쪽을 쓴다.
...
**`brief` 가 "0 명에게 전달" 이라고 한다**

아무에게도 안 갔다는 뜻이다. `brief` 는 **`대기` 인 멤버에게만** 보낸다 —
작업 중이거나 대화상자 앞에 멈춘 사람 화면에 문장을 밀어 넣으면 그 입력이
엉뚱한 곳으로 들어가기 때문이다. 바로 아래 `건너뜀` 줄이 이름별로 이유를 말한다.

| 건너뛴 이유 | 할 일 |
|---|---|
| `(prompt)` 대화상자가 떠 있다 | 붙어서 답한 뒤 다시 `brief` |
| `(starting)` 부팅 중 | 기다렸다가 다시 `brief` |
| `(down)` claude 가 없다 | `./aiorg launch` 먼저 |
| `(busy)` 작업 중 | 끝나길 기다린다 |

전원이 `(prompt)` 라면 대개 **신뢰 확인 대화상자**다. 부서 단위로 붙어서
`Ctrl-b n` 으로 창을 넘겨 가며 답하면 세션 수만큼만 붙으면 된다.

```bash
./aiorg attach dev-lead     # ↓ 로 'Yes, I trust this folder' 고르고 Enter
                            # Ctrl-b n 으로 다음 창 -> 같은 방식으로
                            # 그 부서를 다 answer 했으면 Ctrl-b d
./aiorg status              # 전원 '대기' 가 될 때까지
./aiorg brief
```

**애초에 안 겪는 법** — `launch` 가 띄우기 전에 알려준다.

```
먼저 확인하세요 — 아직 신뢰 확인을 받지 않은 작업 디렉터리가 있습니다:
  cd '/경로/내제품' && claude
```

그 한 줄을 먼저 실행해 한 번만 답해 두면, 이후 여덟 자리가 조용히 뜬다.
`scaffold` 가 권한 설정을 만들거나 고쳐도 다시 묻으므로 **`scaffold` 를
`launch` 보다 먼저** 실행한다.
