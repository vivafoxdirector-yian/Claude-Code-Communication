...
**실측 40분쯤** 걸린다 (URL 단축기 규모, 8명 조직 기준). 손으로 하는 것은
0~7 단계뿐이고 5분이면 끝난다. 나머지는 조직이 일하는 것을 지켜보는 시간이다.

시간이 어디에 쓰이는지 실측값 — 목표를 준 시점부터:

| | 걸린 시간 |
|---|---|
| 대표가 목표를 쪼개 기획·개발·품질에 배정 | 1분 |
| PO 명세서 완료 | 3분 |
| 백엔드 구현·테스트·커밋 | 9분 |
| 프론트엔드 구현·커밋 | 15분 |
| QA 검증 → 지적 4건 → 수정 → 재검증 | 28분 |

QA 가 보안 결함(헤더 인젝션, 약한 난수)을 잡아 개발팀장이 고치고 다시 검증받는
사이클이 돌기 때문에 뒤쪽이 길다. 그게 정상이고, 건너뛰고 싶으면
[10 단계](#10-중간에-끼어들기)처럼 대표에게 말한다.
...
`up` 은 이때 **커밋 가드**도 걸어 둔다 (`.git/hooks/pre-commit`). 구성원이 이
프레임워크 저장소에 커밋하려 하면 거부한다 — 지시서에 못 박아도 방향을 잃고
이쪽에 산출물을 만드는 일이 실제로 있었기 때문이다. 당신(운영자)의 커밋은 통과하고,
멤버 워킹트리(`worktree: true`)의 커밋도 통과한다.
...
## 6. 알림 지킴이 — `up` 이 이미 띄웠다

손으로 할 것이 없다. `up` 이 `<prefix>-notify` 세션에 함께 띄우고,
`down` 이 같이 내린다.

```
알림 지킴이 기동 (aiorg-notify) — 보류된 벨을 계속 재시도합니다
```

왜 있어야 하는지는 알아 둘 만하다. 상대가 작업 중일 때 보낸 메시지는
**벨이 보류된다** (메시지는 수신함 파일에 안전하게 들어가 있다). 지킴이가
상대가 한가해질 때 다시 울려 준다. 없으면 이렇게 된다 —
**전원 `대기` 인데 미확인이 줄지 않는다.** 조직이 조용히 멈춘 것이다.

원래는 사람이 별도 터미널에서 켜 두는 것으로 되어 있었는데, 실제로 잊어서
조직이 멈추는 것을 봤다. 그래서 `up` 이 직접 띄운다.

지킴이가 죽으면 `status` 가 알려준다.

```
경고: 알림 지킴이가 돌고 있지 않습니다. 보류된 벨이 전달되지 않습니다.
      ./aiorg notify        지금 한 번 재시도
      ./aiorg up            지킴이까지 다시 띄움 (살아 있는 세션은 그대로)
```
...
**조직도의 기호가 물음표로 보인다**
글꼴에 이모지 글리프가 없는 것이다. 로케일이 UTF-8 이어도 생기므로 자동으로는
못 잡는다. ASCII 로 바꾼다.
...
계속 그렇게 쓰려면 셸 설정에 `export AIORG_ASCII=1` 을 넣는다.
`@` 대표, `+` 팀장, `-` 개발자, `?` 리뷰어, `=` PO, `!` 프리세일즈.
